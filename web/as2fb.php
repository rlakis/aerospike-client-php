...
//\error_log("POST ". \json_encode($_POST, JSON_PRETTY_PRINT)."\t-> ".$entityBody.$_SERVER['REQUEST_METHOD']);
    ...
    */
    function raw($refresh = false) {
        ...
        if ($this->method==='POST') {
            ...
            $binary = fread($fp, $headers['CONTENT-LENGTH']);
            // finally close the file
            fclose($fp);

            //$wrapped = new \Aerospike\Bytes($binary);
            // unpack the data - notice that we create a format code using 'C%d'
            // that will unpack the size of the file in unsigned chars
            //$unpacked = unpack(sprintf('C%d', $headers['CONTENT-LENGTH']), $binary);
            //\error_log(json_encode($unpacked));
            //error_log(base64_decode($binary));
        }
