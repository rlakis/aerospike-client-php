(function(h) {
    h.fn.passStrengthify = function(a) {
        var q = h(this),
            m, r = [
                [0, 8, 16, 32, 48, 64, 72],
                [0, 16, 32, 48, 64, 78, 92],
            ...
            s = ["Very weak", "Very weak", "Weak", "Weak", "Moderate", "Good", "Strong", "Very strong"],
            n = ["gray", "red", "red", "#C00000", "orange", "#0099FF", "blue", "green"],
            i = r[0],
            u = h("<span>").css("margin-left", "1em"),
            t = [],
            e = 0,
            v = 0,
            w = false,
            B = function(b) {
                ...
                for (j = d = 0; j < b.length; j++) {
                    ...
                    if (typeof p[l] != "undefined") l = p[l];
                    if (l.match(/^[a-zA-Z]$/)) {
                        l = l.toLowerCase().charCodeAt(0) - "a".charCodeAt(0);
                        var g = void 0;
                        g = null;
                        if (o) {
                            g = k.charAt(o - 1);
                            if (typeof p[g] != "undefined") g = p[g]
                        }
                        if (g != null && g.match(/^[a-zA-Z]$/)) {
                            k = void 0;
                            k = g.toLowerCase().charCodeAt(0) - "a".charCodeAt(0);
                            g = A[l + 26 * k]
                        } else g = (o ? y : z)[l];
                        if (f >= 26) g *= 26 / f;
                        f = g
                    } else f = 1 / f;
                    d += Math.max(-7, Math.log(f) / 0.6931471805599453)
                }
                return -1 * d
            },
            C = function(b) {
                var c = 0,
                    d = 0;
                d = B(b);
                if (i.length && i[0] instanceof RegExp) jQuery.each(i, function(j, f) {
                    c += b.search(f) != -1
                });
                else
            ...
            x = function() {
                ...
                var j = t.length,
                    f, k;
                k = c ? a.labels.tooShort : w ? Math.round(d * 100) / 100 + " bits" : s[b];
                f = n[b];
                c = n[b];
                d = n[0];
                u.text(k).css("color", f);
                for (e = 0; e < j; e++) t[e].css("background-color", e < b ? c : d)
            };
        if (typeof a == "undefined") a = {};
        if (typeof a.labels === "undefined") a.labels = {};
        a.labels = h.extend({
            tooShort: "Too short",
            passwordStrength: "Password strength:"
        }, a.labels);
        m = h("<span>").css("display", "inline-block").addClass("passStrengthify");
        return h(this).each(function() {
            a.element ? a.element.append(m) : h(this).parent().append(m);
            if (a.minimum) v = a.minimum;
            if (typeof a.security == "undefined") a.security = 1;
            if (a.security >= 0 && a.security < r.length) i = r[a.security];
            if (!a.levels) a.levels = s;
            if (!a.colours) a.colours = n;
            if (!a.tests) a.tests = i;
            if (a.levels && a.colours && a.tests)
                if (a.levels.length == a.colours.length && a.colours.length == a.tests.length + 1) {
                    s = a.levels;
                    n = a.colours;
                    i = a.tests
                }
            if (a.rawEntropy) w = true;
            m.append(h("<div>").append(h("<span>").css("font-size", "smaller").text(a.labels.passwordStrength).append(u)));
            var b = Math.round((125 - 3 * i.length) / i.length);
            for (e = 0; e < i.length; e++) {
                var c = h("<span>").css("height", "3px").css("width", b + "px").css("margin-right", "3px").css("max-height", "3px").css("font-size", "1px").css("float", "left");
                t.push(c);
                m.append(c)
            }
            q.keypress(x);
...
var pOpts = {
    minimum: 6,
    labels: {
        tooShort: (lang == 'ar' ? 'الحد الأدنى 6 أحرف' : 'minimum of 6 characters'),
        passwordStrength: ""
    },
    element: $('#pout')
};
if (lang == 'ar') {
    pOpts['levels'] = ['ضعيف جداً', 'ضعيف جداً', 'ضعيف', 'ضعيف', 'لا بأس', 'جيد', 'قوي', 'قوي جداً']
}
$("#pwd").passStrengthify(pOpts);