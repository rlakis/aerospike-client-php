...
\error_log(\sprintf("%s\t%s", \date("Y-m-d H:i:s"), 'PFIPN: '.\json_encode($_POST).PHP_EOL), 3, $logfile);
...
$orderId=$userId=$sourceId=0;

if (isset($payment['merchant_reference'])) {
    $order=\preg_split('/-/', $payment['merchant_reference']);
    if (\is_array($order) && (\count($order)===2||\count($order)===3) && \is_numeric($order[0]) && \is_numeric($order[1])) {
        if (isset($order[2])) {  $sourceId=(int)$order[2];  }
        $userId=(int)$order[0];
        $orderId=(int)$order[1];
    }
    else {
